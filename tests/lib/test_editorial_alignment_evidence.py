...
def test_alignment_evidence_binds_output_timeline_and_current_sources(tmp_path: Path):
    ...
    timeline = {"tracks": [{"kind": "video", "clips": [{"id": "video-1", "fact_scope": {
    ...
    evidence = build_editorial_alignment_evidence(
        ...
        checks=[{"clip_id": "video-1", "shot_id": "shot-1", "scene_id": "scene-1", "action_match": "pass",
...
def test_alignment_evidence_requires_each_video_clip_check(tmp_path: Path):
    from lib.editorial_alignment_evidence import build_editorial_alignment_evidence

    timeline = {"tracks": [{"kind": "video", "clips": [
        {"id": "clip-1", "fact_scope": {"shot_id": "shot-1", "claim_ids": ["claim-1"], "visual_requirement_id": "visual-1"}},
        {"id": "clip-2", "fact_scope": {"shot_id": "shot-1", "claim_ids": ["claim-1"], "visual_requirement_id": "visual-1"}},
    ]}]}
    output = tmp_path / "preview.mp4"; output.write_bytes(b"new")
    evidence = build_editorial_alignment_evidence(
        timeline={**timeline, "timeline_hash": canonical_digest(timeline)}, output_path=output,
        output_probe={"duration_seconds": 1}, frame_samples=[{"timestamp_seconds": 0}],
        fact_bindings=[{"clip_id": "clip-1", "shot_id": "shot-1", "claim_ids": ["claim-1"], "visual_requirement_id": "visual-1"},
                       {"clip_id": "clip-2", "shot_id": "shot-1", "claim_ids": ["claim-1"], "visual_requirement_id": "visual-1"}],
        script_hash="b" * 64, product_facts_hash="c" * 64, visual_requirements=[{"id": "visual-1"}],
        checks=[{"clip_id": "clip-1", "shot_id": "shot-1", "status": "pass"}],
    )
