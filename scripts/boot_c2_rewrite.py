"""c2 引导（research 已由 fork 播种）：run plan → proposal 种子 → rebuild → 批准 → 资产同步 → assets 门。"""
import sys, json
sys.path.insert(0, '.')
...
ROOT = Path('.')
RUN = 'template-run-sheet-09-video9-aks-zhuodian-c2'
DD = ROOT / 'projects' / RUN
SRC = ROOT / 'projects/template-run-sheet-09-video9-aks-zhuodian'
PACK_DIR = ROOT / 'projects/template-pack-library'
pack = json.load(open(PACK_DIR / 'artifacts/template_pack.json'))
t = next(x for x in pack['templates'] if x['template_id'] == 'sheet-09-video9-aks-zhuodian-c2')
...
if not (DD / 'artifacts/template_run_plan.json').exists():
    ...
    rp = create_template_run(t, template_pack_ref={'artifact_sha256': attach_hashes(dict(pack))['artifact_sha256'], 'version': '1.0'},
                             product_facts_ref={'artifact_sha256': facts.get('artifact_sha256') or attach_hashes(dict(facts))['artifact_sha256']},
    ...
    with ProjectCommitStore(DD).transaction(action={'action_id': 'seed-c2-rp'}) as sink:
        ...
        for name in ('proposal_packet.json', 'creative_control_plan.json', 'hook_plan.json', 'decision_log.json'):
            ...
            amap[name.split('.')[0]] = {'name': name.split('.')[0], 'path': f'artifacts/{name}',
                                        'semantic_sha256': data['semantic_sha256'], 'artifact_sha256': data['artifact_sha256'],
                                        'data': data}
        ...
        write_artifact_atomic('artifacts/product_facts.json', 'product_facts', attach_hashes(dict(facts)),
                              project_dir=DD, sink=sink)
    print('run plan + proposal seeded')

from lib.template_mainline import rebuild_aligned_run
rebuild_aligned_run(RUN)
print('rebuild done (script/scene_plan, semantic gate)')

with ProjectCommitStore(DD).transaction(action={'action_id': 'c2-sync'}) as sink:
    rp = json.load(open(DD / 'artifacts/template_run_plan.json'))
    if rp.get('status') != 'approved':
        rp['status'] = 'approved'
        rp.pop('artifact_sha256', None); rp.pop('semantic_sha256', None)
        env = write_artifact_atomic('artifacts/template_run_plan.json', 'template_run_plan', attach_hashes(dict(rp)),
                                    project_dir=DD, sink=sink)
...
with ProjectCommitStore(DD).transaction(action={'action_id': 'c2-assets-gate'}) as sink:
    amap2 = {}
    for name in ('asset_plan.json', 'production_lock.json', 'approval_bundle.json'):
        key = name.split('.')[0]
        data = dict(json.load(open(SRC / 'artifacts' / name)))
        data['project_id'] = RUN
        data['input_hashes'] = {'base_run_plan_sha': rplan_sha}
