...
2. **产品事实卡（建批前）**：按 `skills/meta/product-fact-card.md` 向用户收集
   一张 SKU/价格/参数卡，写入批根 `projects/<batch_id>/artifacts/product_facts.json`；
   `batch_fork` 会复制给每个候选（L1a 事实检查据此从 skip 变 pass）。
3. **建批**：`lib.candidate_batch.create_candidate_batch(...)` 写入
...
4. **分叉**：为每个候选创建独立项目（`projects/<project_id>`），分叉函数先
   ...
   不复制重做。
5. **审批门**：creative lock bundle 必须包含候选差异计划；计划状态为
   `awaiting_human` 时不得进入付费素材或样片调用。warning 模式只放宽差异
   不足的阻塞，不放宽人工审批。
6. **并发纪律**：同一时刻最多 `concurrency.max_parallel`（2-3）个候选在跑；
...
7. **排序**：统一比较各候选 evaluation_report（hard_gate / 分数 / 失败项）、
   成本、执行差异与样片前 3 秒；排序结果写入 batch notes（或审核台展示）。
8. **人工选择**：用户在审核台选 1-2 条 → `select_for_edit(...)`（规则：必须
