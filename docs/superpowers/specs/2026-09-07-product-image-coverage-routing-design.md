...
新采集使用 `ProductPageCapture 1.1`。只有以下五个页面面均有证据时，
`acquisition_status` 才能为 `complete`：商品身份、URL 对应的选中 SKU、
参数表、完整主图组、详情内容。`capture_scope` 同时记录页面事实总数、
明确排除的易变事实数以及每个页面面的截图或 ledger 资产引用。

- 主图/详情图里的营销文案也进入事实候选，但默认保持 `page_claim`；
- 检测、认证、抗菌等级、安全性、比较级等高风险表述，没有报告正文与
  SKU 适用范围时不得升级为独立检测事实；
- 价格、优惠、库存、发货时效等易变内容可以采集，但必须标记
  `volatile=true`，调和后不进入复用脚本；
- 旧 `1.0` 产物仍可读取，但不能作为新采集完整性的证明；下一次浏览器
  重采应升级为 `1.1`。
