...
本阶段升级为运营审核台。详细数据合同、交互和验收标准见
`2026-08-19-backlot-delivery-review-workbench-design.md`。

页面必须显示：

- 带真实 poster 的成片播放器；
- 画面、口播、文案、BGM/SFX 四轨审核时间线；
- 按完整语义句组织、可跨多个镜头的文案；
- 封面、前三秒、BGM 和结尾的模块化候选；
- V1/V2/V3 完整版本切换和业务变更摘要；
- 暂存修改、查看影响和生成新版的受约束操作；
- 生成进度；
- QA 项目及中文结果；
...
候选只能使用自有素材、本项目生成资产和当前成片。参考视频继续只用于分析，
不得进入封面、候选、时间线媒体或交付文件。修改先进入草稿并查看影响；系统只在
用户确认后生成一个完整新版，新版必须通过 QA 才能成为当前版。
