...
样片确认还必须完成一组效果确认，不能只因为视频已经生成或 QA 通过就自动进入下一步。首期固定确认五项：

1. **创意方向**：是否仍然围绕已锁定的核心卖点和差异化方向。
2. **开头钩子**：前 1-3 秒是否交代清楚并有继续观看的理由。
3. **核心证明**：产品、动作、结果和卖点是否形成清晰闭环。
4. **节奏与画面切换**：镜头时长、剪辑点和切换是否顺畅，没有拖沓、跳跃或重复。
5. **字幕与画面可读性**：字幕、产品主体和重点信息是否清楚，不遮挡动作、不超出安全区。

每项必须选择“通过 / 需要调整 / 方向不对”。只有五项全部选择“通过”，才允许提交“确认样片并进入下一步”；“需要调整”进入精剪修改，“方向不对”返回创意方案或导演总控单。效果确认结果与样片审核结果写入同一审核记录，刷新或重启后不得丢失。
...
### 5.7 修改与精剪阶段：局部返工、影响可见、自动回到样片确认

#### 5.7.1 阶段定位

修改与精剪不是重新创作，也不是重新打开导演总控单。它只处理样片确认后
的局部返工，目标是让一线制作人员可以快速修正节奏、镜头和声音，同时保
护已经锁定的内容方向、故事结构、视觉规则、事实连续性和原创边界。

首期允许修改：

| 可修改内容 | 用户语言 | 影响类型 |
|---|---|---|
| 镜头取舍 | 保留 / 删除这一镜 | 画面重排，重新生成样片 |
| 素材区间 | 这一镜从第几秒到第几秒 | 画面时长变化，重新生成样片 |
| 播放速度 | 这一镜快一点 / 恢复原速 | 画面节奏变化，重新生成样片 |
| 字幕 | 改短、改清楚、换重点词 | 画面重渲染，重新确认样片 |
| 声音 | 音乐小一点、音效小一点、关闭口播 | 只更新声音时可走 `mux_only` |

以下要求必须回到上游阶段：

- 改核心卖点、创意方向或导演规则：回到创意方案 / 导演总控单；
- 改故事顺序、口播结构或整段文案：回到剧本生成；
- 换素材、重排分镜或改变镜头任务：回到分镜 / 制作准备；
- 改画幅、构图体系或原创边界：回到导演总控单。

#### 5.7.2 快速修改交互

页面以横向镜头卡为主要编辑入口。每张卡显示当前素材、区间、速度、字幕、
镜头状态和局部预览，并提供固定快捷动作：
...
快捷动作只写入当前用户的 `edit` 草稿，不立即改正式产物，也不立即触发渲染。
连续修改可以合并提交，避免每次输入都生成样片。页面保留“恢复本轮修改”，
便于用户在影响预览前撤销草稿。

P1 再增加业务快捷动作：“删除重复镜头”“开头节奏加快”“只保留产品证明镜头”、
“统一降低背景音乐”。这些动作必须展开为标准 `edit-v1` 操作，不能绕过草稿、
版本和影响预览。

#### 5.7.3 三级影响预览

点击“查看修改影响”后，右侧固定显示影响面板，不让用户在长页面中寻找结果。
面板分三层：

1. **一句话结论**：例如“本次修改涉及 3 个镜头，需要重新生成画面，不改变创意方向。”
2. **镜头明细**：显示每个受影响镜头、修改前后值和结果，例如“shot-02 素材缩短
   0.6 秒 → 重新渲染这一镜”。
3. **阶段影响**：明确显示“修改与精剪写入新版本”“样片确认重新打开”“创意方案、
   剧本、分镜不受影响”，以及预计时间、预计费用和警告。

影响预览必须继续使用现有 `ImpactPreview` 契约，并补齐 `edit` 的镜头级
`affected_scene_ids`。渲染路线固定为：

- 无实际变化：无需重新生成视频；
- 仅声音变化：保留画面，仅更新声音；
- 镜头、区间、速度或字幕变化：重新生成完整画面。

预览 token 绑定草稿内容、用户和当前 generation。草稿变化、项目版本变化或
token 过期，都必须要求重新预览，禁止使用旧影响直接提交。

#### 5.7.4 提交后的自动链路

用户点击“确认修改并生成新版”后，在同一个事务中完成：

1. 写入新的 `edit_decisions` revision；
2. 写入 `change_impact`，绑定修改前后 hash、受影响镜头和渲染路线；
3. 标记本次 `edit` 修订已提交；
4. 创建新版样片生成任务或持久化待执行事件；
5. 将样片 checkpoint 置为 `in_progress`，写入可恢复的 `next_action`；
6. 追加“正在生成新版样片”的 Agent / 编排事件。

用户看到的状态是：

> 修改已提交，正在生成新版样片。新版生成后需要重新确认效果。

当前没有直连 Code Agent 时，系统必须持久化任务和继续指令，不得假装已经完成
生成。Code Agent 恢复时直接读取 `next_action` 执行，不要求用户重新描述修改内容。
新版样片必须再次经过五项效果确认，不能从精剪直接进入最终成片。

#### 5.7.5 当前需要补齐的契约

当前页面和 `EditAdapter` 已暴露编辑能力，但 `operator_draft.schema.json` 尚未
完整纳入 `edit` 阶段。P0 必须统一补齐：

- `stage: edit` 与 `adapter: edit-v1`；
- `set_shot_enabled`、`set_source_range`、`set_shot_speed`、`set_caption`、
  `set_audio_mix` 操作联合定义；
- 草稿保存、影响预览、提交和版本历史的 `edit` API 链路；
- 无效镜头、无效区间、非正速度、全部镜头被删除等校验；
- 提交后的样片 checkpoint、队列事件和 `next_action` 原子写入。

#### 5.7.6 验收标准

- 用户可以在镜头卡上连续完成删镜头、改区间、改速度和改字幕，并暂存为一份草稿；
- 影响预览同时显示摘要、受影响镜头和受影响阶段；
- 仅调声音时显示“保留画面，仅更新声音”，不标记画面镜头；
- 修改镜头、区间、速度或字幕时显示“重新生成完整画面”，并重新打开样片确认；
- 提交后刷新或重启，仍能看到“新版样片生成中”和可执行的下一步；
- 新版样片完成后回到样片确认，而不是直接进入成片生成；
- 修改核心方向、剧本结构或分镜任务时，页面明确提示返回上游阶段；
- `table-mat-mix-v7` 完成一次“删镜头 → 查看影响 → 提交 → 生成新版样片”的端到端回归。
