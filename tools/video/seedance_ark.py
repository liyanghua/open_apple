...
class SeedanceArkVideo(BaseTool):
    ...
    def _get_api_key(self) -> str | None:
        # Accept both the canonical Ark names and the user's existing
        # OpenMontage/Doubao names.  The latter keeps the provider compatible
        # with the .env already configured for this project.
        return os.environ.get("ARK_API_KEY") or os.environ.get("DOUBAO_LITE_API_KEY")
    ...
    def _get_base_url(self) -> str:
        ...
        # The existing .env points at the Responses surface.  Video tasks use
        # the sibling contents/generations/tasks resource under /api/v3.
        if base_url.endswith("/responses"):
            base_url = base_url[: -len("/responses")]
        if not base_url.startswith("https://"):
            raise ValueError("ARK_BASE_URL/DOUBAO_BASE_URL must be an https:// URL")
        return base_url
    ...
    def _resolve_model(self, inputs: dict[str, Any]) -> tuple[str, str | None]:
        ...
        model = str(
            ...
            or os.environ.get("DOUBAO_SEEDANCE_MODEL")
