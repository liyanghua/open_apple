...
const STAGE_MATERIALS = {
  ...
  assets: [
    ["generation_list", "素材处理清单", ["execution_plan", "generation_list", "shots"]],
...
function compactScenePlan(data) {
  ...
  return {
    ...
    shots: shots.map((shot) => ({
      ...
      claim_ids: Array.isArray(shot?.claim_ids) ? shot.claim_ids : [],
      action_keys: Array.isArray(shot?.action_keys) ? shot.action_keys : [],
      evidence_row_ids: Array.isArray(shot?.evidence_row_ids) ? shot.evidence_row_ids : [],
      subject_completeness: shot?.subject_completeness,
      crop_strategy: shot?.crop_strategy,
      caption_safe_zone: shot?.caption_safe_zone,
...
function compactGenerationList(data) {
  const items = Array.isArray(data.items) ? data.items.map((item) => ({
    ...
    screen_copy: item?.screen_copy,
    claim_ids: Array.isArray(item?.claim_ids) ? item.claim_ids : [],
    action_keys: Array.isArray(item?.action_keys) ? item.action_keys : [],
    evidence_row_ids: Array.isArray(item?.evidence_row_ids) ? item.evidence_row_ids : [],
    product_fact_refs: Array.isArray(item?.product_fact_refs) ? item.product_fact_refs : [],
    product_page_refs: Array.isArray(item?.product_page_refs) ? item.product_page_refs : [],
    page_asset_ids: Array.isArray(item?.page_asset_ids) ? item.page_asset_ids : [],
    page_evidence_ids: Array.isArray(item?.page_evidence_ids) ? item.page_evidence_ids : [],
    fact_bindings: Array.isArray(item?.fact_bindings) ? item.fact_bindings.map((fact) => ({
      ref: fact?.ref,
      ...
      sku_scope: Array.isArray(fact?.sku_scope) ? fact.sku_scope : [],
      allowed_wording: Array.isArray(fact?.allowed_wording) ? fact.allowed_wording : [],
      prohibited_wording: Array.isArray(fact?.prohibited_wording) ? fact.prohibited_wording : [],
      provenance_refs: Array.isArray(fact?.provenance_refs) ? fact.provenance_refs : [],
    })) : [],
    fact_role: item?.fact_role,
    page_evidence: Array.isArray(item?.page_evidence) ? item.page_evidence.map((evidence) => ({
      id: evidence?.id,
      label: evidence?.label,
      kind: evidence?.kind,
      usage_role: evidence?.usage_role,
    ...
    evidence_role_label: item?.evidence_role_label,
    visual_requirement: item?.visual_requirement && typeof item.visual_requirement === "object" ? {
      visualizability: item.visual_requirement.visualizability,
      required_subjects: Array.isArray(item.visual_requirement.required_subjects) ? item.visual_requirement.required_subjects : [],
      required_actions: Array.isArray(item.visual_requirement.required_actions) ? item.visual_requirement.required_actions : [],
      required_results: Array.isArray(item.visual_requirement.required_results) ? item.visual_requirement.required_results : [],
      forbidden_substitutions: Array.isArray(item.visual_requirement.forbidden_substitutions) ? item.visual_requirement.forbidden_substitutions : [],
      risk_level: item.visual_requirement.risk_level,
    } : {},
    owned_candidates: Array.isArray(item?.owned_candidates) ? item.owned_candidates.map((candidate) => ({
      media_id: candidate?.media_id,
      source_path: candidate?.source_path,
      source_range: candidate?.source_range,
      confidence: candidate?.confidence,
      status: candidate?.status,
      observed_actions: Array.isArray(candidate?.observed_actions) ? candidate.observed_actions : [],
      observed_results: Array.isArray(candidate?.observed_results) ? candidate.observed_results : [],
      subject_complete_in_3_4: candidate?.subject_complete_in_3_4 === true,
      rejection_reasons: Array.isArray(candidate?.rejection_reasons) ? candidate.rejection_reasons : [],
      preview_url: candidate?.preview_url,
    })) : [],
    generation_reference: item?.generation_reference && typeof item.generation_reference === "object" ? {
      source_path: item.generation_reference.source_path,
      preview_url: item.generation_reference.preview_url,
      original_source_path: item.generation_reference.original_source_path,
      original_preview_url: item.generation_reference.original_preview_url,
      sku_scope: Array.isArray(item.generation_reference.sku_scope) ? item.generation_reference.sku_scope : [],
      identity_status: item.generation_reference.identity_status,
      ocr_clean: item.generation_reference.ocr_clean === true,
      generation_eligibility: item.generation_reference.generation_eligibility,
    } : null,
    generation_plan: item?.generation_plan && typeof item.generation_plan === "object" ? {
      operation: item.generation_plan.operation,
      prompt: item.generation_plan.prompt,
      duration_seconds: item.generation_plan.duration_seconds,
      aspect_ratio: item.generation_plan.aspect_ratio,
      required_actions: Array.isArray(item.generation_plan.required_actions) ? item.generation_plan.required_actions : [],
      required_results: Array.isArray(item.generation_plan.required_results) ? item.generation_plan.required_results : [],
      prohibitions: Array.isArray(item.generation_plan.prohibitions) ? item.generation_plan.prohibitions : [],
      retry_limit: item.generation_plan.retry_limit,
      selection_status: item.generation_plan.selection_status,
    } : null,
    generation_options: Array.isArray(item?.generation_options) ? item.generation_options.map((option) => ({
