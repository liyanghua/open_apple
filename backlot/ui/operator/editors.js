...
export function renderTypedEditor(container, stage, editor, { editable, onOperation }) {
  ...
  if (editor?.type === "proposal_choice") {
  ...
  } else if (editor?.type === "script_editor") {
    (data.sections || []).forEach((section) => add(`${section.label || "段落"} 口播`, textarea(section.text, (text) => emit({ op: "replace_section_narration", section_id: section.id, text }))));
    ...
    (data.shots || []).forEach((shot) => {
      ...
      input.addEventListener("input", () => emit({ op: "set_source_range", shot_id: shot.id, in_seconds: Number(shot.in_seconds || 0), out_seconds: Number(input.value) }));
  ...
  } else if (editor?.type === "research_review") {
    add("业务备注", textarea("", (text) => emit({ op: "set_business_note", target_id: "project", text })));
  } else {
